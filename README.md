...
- **Личный кабинет** — профиль и смена пароля у каждого пользователя; у администратора внутри же управление участниками.
...
В настройках проекта указывается вебхук канала и что отправлять — краши, обращения или и то и другое. Приходят данные от игры — в канал уходит сообщение со ссылкой на карточку в панели.

Если Discord с вашего сервера недоступен (так бывает у российских хостеров), пропишите прокси — он используется только для уведомлений:

```json
"Project": {
  "Notifications": { "ProxyUrl": "http://user:pass@proxy-host:3128" }
}
```

Поддерживаются `http://` и `socks5://`. Переменная окружения — `Project__Notifications__ProxyUrl`. Проверить связь можно кнопкой «Отправить тест» в настройках проекта.
...
CrushHUB/
  Controllers/
    ...
    Cabinet/        личный кабинет: профиль, безопасность, пользователи (partial-части CabinetController)
...
Загруженные скриншоты (`wwwroot/uploads`) 
